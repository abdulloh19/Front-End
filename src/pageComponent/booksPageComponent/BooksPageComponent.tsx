...
const BooksPageComponent = () => {
  ...
  const bagcroundcolor = useColorModeValue("gray.200", "gray.900");
  ...
  return (
    <>
      ...
      <Grid mb={20} gridTemplateColumns={{ base: "repeat(1, 1fr)", md: "repeat(2, 1fr)", lg: "repeat(3, 1fr)" }} gap={4} mt={5} rowGap={20}>
        {filteredData().map((item) => (
          ...
          >
            <Box pos={"relative"}>
              <Image src={item.image} alt={item.name} h={"250px"} w={"full"} borderRadius={"lg"} objectFit={"cover"} />
              <HStack
                pos={"absolute"}
                minH={"90px"}
                borderRadius={"lg"}
              ...
              >
                <Box>
                  <Text fontSize={"md"}>{t(item.name, { ns: "books" })}</Text>
                  <Text fontWeight={"bold"} fontSize={"2xl"}>
                    {item.price.toLocaleString("en-US", { style: "currency", currency: "USD" })}
                  </Text>
                </Box>
                <Button colorScheme="facebook" rightIcon={<AiFillShopping />}>
                  {t("filter_by_button", { ns: "books" })}
                </Button>
              </HStack>
            </Box>
          </motion.div>
